...
#add stim type interactions for the musicality + liking ratings 
#pull out betas from participant level
```
...
SE_musicality_pre$freq <-table(master_long_pre$musicality)
...
SE_musicality_post$freq <-table(master_long_post$musicality)
...
SE_liking_pre$freq <-table(master_long_pre$liking)
...
SE_liking_post$freq <-table(master_long_post$liking)

SE_liking_post$SE_liking <- (SE_liking_post$liking_sd)/(sqrt(SE_liking_post$freq))

SE_liking_post <- merge(SE_liking_post, liking_means_post, by=c("musicality", "pre_post")) 


SE_liking_pre <- merge(SE_liking_pre, liking_means_pre, by=c("musicality", "pre_post")) 

SE_liking <- rbind(SE_liking_post, SE_liking_pre)

names(SE_liking)[1]<-"index"
SE_liking$lower_liking <- SE_liking$liking-SE_liking$SE_liking
SE_liking$upper_liking <- SE_liking$liking+SE_liking$SE_liking

SE <- merge(SE_liking, SE_musicality, by=c("index","pre_post"))
