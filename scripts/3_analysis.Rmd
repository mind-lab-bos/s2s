...
summary(m1)
test(emmeans(m1, ~ location*stim_type*pre_post, var = "stim_type", lmer.df = "satterthwaite",lmerTest.limit = 85652))

test(emmeans::emtrends(m1, pairwise ~ value, var = "location", lmer.df = "satterthwaite",lmerTest.limit = 85652))


m1 <- lmer(data=liking_master, scale(value) ~ stim_type*pre_post +  (stim_type*pre_post|ResponseId), control=lmerControl(optimizer="bobyqa"))
summary(m1)

m1 <- lmer(data=speech_master, scale(value) ~ stim_type*pre_post + (stim_type*pre_post|ResponseId) + (1|stim) + (1|location), control=lmerControl(optimizer="bobyqa"))
summary(m1)
...
#potentially plot change as on same axis (violin/bar plot)
#liking_musciality_all <-
  change_master_plot <- change_master %>%
    pivot_longer(liking_change:musicality_change, names_to = "Rating", values_to = "Change")
    
change_bar <- 
  change_master_plot %>% 
  mutate(Rating = recode(Rating, "liking_change" = "Liking", "musicality_change" = "Musicality")) %>%
  #mutate(rating = factor(rating, levels = c("Musicality", "Liking"))) %>%
  mutate(stim_type = recode(stim_type, "illusion" = "Illusion", "control" = "Control")) %>%
  ggplot(aes(x=Rating, y=Change, color=stim_type, fill=stim_type)) + 
  #geom_violin() +
  stat_summary(fun.data = 'mean_cl_normal', geom = 'errorbar', width = 0.05, alpha=0.5, aes(color=)) +
  stat_summary(fun.data = 'mean_cl_normal', geom = 'pointrange', alpha=0.5)+
  theme_bw()+ 
  #geom_jitter(width=0.4,height=0.4,alpha=0.1) +
  #geom_line(alpha=0.05, color="black")+
...
liking_master_birkbeck <- subset(liking_master, location=="birkbeck")

m2 <- lmer(data=liking_master, scale(value) ~ stim_type*pre_post + (stim_type*pre_post|ResponseId) + (1|stim) + (1|location),control=lmerControl(optimizer="bobyqa"))
summary(m2)
...
speech_matched$labels <- factor(speech_matched$pre_post, levels = c("pre", "post"),
                  labels = c("Pre Repetition", "Post Repetition")
                  )
speech_matched$stim_type <- ifelse(speech_matched$stim_type=="control", "Control", "Illusion")

ggplot(data=speech_matched, aes(x=labels, y=value, color=group))+ 
 # ggdist::geom_dotsinterval() +
  stat_summary(fun.data = 'mean_cl_normal', geom = 'errorbar', width = 0, alpha=0.5) +
  stat_summary(fun.data = 'mean_cl_normal', geom = 'pointrange', alpha=0.5) +
  theme_classic()+ 
...
m1a <- lmer(data=liking_matched, scale(value) ~ stim_type*group + (1|ResponseId) + (1|stim),control=lmerControl(optimizer="bobyqa"))
summary(m1a)

m1b <- lmer(data=liking_matched, scale(value) ~ group*pre_post + (group|ResponseId),control=lmerControl(optimizer="bobyqa"))
summary(m1b)

test(emmeans(m1b, pairwise ~ pre_post*group,lmer.df = "satterthwaite",lmerTest.limit = 85652))
...
m10 <- lmer(data=speech_master, scale(value) ~ Q409*pre_post + (Q409 + pre_post|ResponseId), control=lmerControl(optimizer="bobyqa"))
summary(m10)
